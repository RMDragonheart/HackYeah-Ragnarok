/*
Kod �r�d�owy funkcji do transformacji wsp�rz�dnych elipsoidalnych na p�askie odwzorowa� kartograficznych UTM, 1992, 2000
...
//=======================================================================
//  Funkcja s�u�y do konwersji wsp�rzednych elipsoidalnych B, L (lat/lon) na p�askie X-northing, Y-easting odwzorowania kartograficznego UTM (dla dowolnej elipsoidy)
//=======================================================================
//      Argumenty wej�ciowe i wyj�ciowe:
//      --------------------------------
//       double a: d�ugo�� du�ej p�osi elipsoidy odniesienia, w metrach (np. dla elipsoidy WGS 84, 6378137.0)
//       double f: sp�aszczenie elipsoidalne (np. dla elipsoidy WGS 84, 1 / 298.257223563)
//       int& utmXZone: nr strefy UTM wg. podzia�u po�udnikowego (zwracane numery od 1 do 60, ka�da strefa ma sze�� stopni)
//       char& utmYZone: nr strefy wg. podzia�u r�wnole�nikowego (zwracane warto�ci: CDEFGHJKLMNPQRSTUVWX)
//       double& easting: wsp�rz�dna Y UTM, w metrach po konwersji [metry]
//       double& northing: wsp�rz�dna X UTM, w metrach po konwersji [metry]
//       double lat, double lon: wsp�rz�dne lat/lon do konwersji [stopnie]
//=======================================================================
void LatLonToUtm (double a, double f, int& utmXZone, char& utmYZone, double& easting, double& northing, double lat, double lon)
{
        // Wsp�czynnik zniekszta�cenia skali w po�udniku osiowym
	...
	if(lon <= 0.0)
        ...
        if(lat < 84.0 && lat >= 72.0)
          {
          // Specjalne zatrzymanie: strefa X ma 12 stopni od p�nocy do po�udnia, nie 8
          utmYZone = cArray[19];
          }
          else
          {
	...
	if(lat >= 84.0 || lat < -80.0)
          {
          // B��dna warto�� szeroko�ci geograficznej (zwracany znak gwiazdki)
           utmYZone = '*';
          }
...
//=======================================================================
//  Funkcja s�u�y do konwersji wsp�rzednych elipsoidalnych B, L (lat/lon) na p�askie X-northing, Y-easting odwzorowania kartograficznego 1992 i 2000 (dla dowolnej elipsoidy)
//=======================================================================
//      Argumenty wej�ciowe i wyj�ciowe:
//      --------------------------------
//       double a: d�ugo�� du�ej p�osi elipsoidy odniesienia, w metrach (np. dla elipsoidy WGS 84, 6378137.0)
...
{
        // Wsp�czynnik zniekszta�cenia skli mapy w po�udniku osiowym dla odwzorowania kartograficznego 2000
        double ok =  0.999923;
        //Wsp�czynnik zniekszta�cenia skli mapy w po�udniku osiowym dla odwzorowania kartograficznego 1992
        if(proj==1) ok =  0.9993;
	double recf;
	double b;
	double eSquared;
	...
	double nfn;
        ...
        if(lon < 13.5 || lon > 25.5)
         {
		//B��dna warto�� d�ugo�ci geograficznej (zwracana warto�� 99999999999999)
...
//=======================================================================
//  Funkcja do konwersji wsp�rz�dnych p�askich X/Y UTM na elipsoidalne lat/lon (dla dowolnej elipsoidy)
//=======================================================================
//      Wymagania:
//      -------------------------------------
//      utmXZone musi by� warto�ci� w garanicach od 1 do 60
//      utmYZone musi by� jedn� z liter: CDEFGHJKLMNPQRSTUVWX
//      Argumenty wej�ciowe i wyj�ciowe:
//      ------------------------------------
//      double a: d�ugo�� du�ej p�osi, w metrach (np. dla elipsoidy WGS 84, 6378137.0)
//      double f: sp�aszczenie elipsoidalne (np. dla elipsoidy WGS 84, 1 / 298.257223563)
//      int utmXZone: nr strefy UTM wg. podzia�u po�udnikowego (zwracane numery od 1 do 60, ka�da strefa ma sze�� stopni)
//      char utmYZone: nr strefy wg. podzia�u r�wnole�nikowego (zwracane warto�ci: CDEFGHJKLMNPQRSTUVWX)
//      double easting, double northing: wsp�rz�dna X, Y UTM do konwersji [metry]
//      double& lat,  double& lon: wsp�rz�dne elipsoidalne lat/lon po konwersji [stopnie]
//=======================================================================
void UtmToLatLon (double a, double f, int utmXZone, char utmYZone, double easting, double northing, double& lat, double& lon)
{
        // Wsp�czynnik zniekszta�cenia skli mapy w po�udniku osiowym dla odwzorowania kartograficznego UTM
        static const double ok = 0.9996;
	double recf;
	double b;
	double eSquared;
...
//=======================================================================
//  Funkcja do konwersji wsp�rz�dnych p�askich X/Y odwzorowania kartograficznego 1992 i 2000 na elipsoidalne lat/lon (dla dowolnej elipsoidy)
//=======================================================================
//      double a: dl�ugo�� du�ej p�si, w metrach (np. dla elipsoidy WGS 84, 6378137.0)
//      double f: sp�aszczenie elipsoidalne (np. dla elipsoidy WGS 84, 1 / 298.257223563)
//      double easting, double northing: wsp�rz�dna X, Y  odwzorowania kartograficznego 1992 lub 2000 do konwersji [metry]
//      double& lat,  double& lon: wsp�rz�dne elipsoidalne lat/lon po konwersji [stopnie]
//      int proj: odwzorowanie kartograficzne (proj = 1 odpowiada odwzorowaniu 1992, natomiast ka�da inna odwzorowaniu 2000)
//=======================================================================
...
//==============================================================================
// Funkcja do konwersji wsp�rz�dnych X/Y 1992 lub 2000 na lat/lon elipsoide WGS 84
//==============================================================================
...
//==============================================================================
// Funkcja do konwersji wsp�rz�dnych lat/lon WGS 84 na X/Y UTM
//==============================================================================
...
//==============================================================================
// Funkcja do konwersji wsp�rz�dnych lat/lon WGS 84 na X/Y 1992 lub 2000
//==============================================================================
void LatLonToPUWGWGS84(double& easting, double& northing, double lat, double lon, int proj)
   {
   //proj = 1 - dla odwzorowania kartograficznego 1992, ka�da inna warto�� dla 2000
   LatLonToPUWG (6378137.0, 1 / 298.257223563, easting, northing, lat, lon, proj);
   }


   void GPStoPL2000(double& easting, double& northing, double lat, double lon)
   {
	int proj = 2;
	LatLonToPUWG (6378137.0, 1 / 298.257223563, easting, northing, lat, lon, proj);
   }
...
//==============================================================================
//  Funkcja do konwersji wsp�rz�dnych X/Y UTM na lat/lon elipsoidalne WGS 84
//==============================================================================
